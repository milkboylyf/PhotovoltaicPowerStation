...
app_train = pd.read_csv('data/public.train.csv')  # 读取训练数据
app_test = pd.read_csv('data/public.test.csv')  # 读取测试数据

train_id = app_train[['ID']]  # 获取训练id
test_id=app_test[['ID']]  # 获取测试id

app_train_test = [app_train, app_test]  # 合并测试和训练数据
...
app_train_test.to_csv('data/data_prc.csv', index=False)
...
app_train.to_csv('data/train_prc.csv', index=False)
app_test.to_csv('data/test_prc.csv', index=False)